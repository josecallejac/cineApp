...
// La lógica de scrape + enriquecido TMDB vive en ./cinepolis (reutilizada por el
// script scripts/push-billboard.js). Aquí solo orquestamos el caché en memoria, el
// snapshot persistente y la cadena de fallbacks.
const { scrapeCinepolisMovies, enrichWithTmdb } = require('./cinepolis');
const fs = require('fs');

const CINEPOLIS_CACHE_TTL_MS = 10 * 60 * 1000;
let cinepolisCache = { movies: null, fetchedAt: 0, pending: null };

// Snapshot persistente de la cartelera. En prod el WAF de Cinépolis bloquea con 403
// la IP de datacenter de Railway, así que el live scrape solo funciona desde una IP
// no bloqueada (la PC del usuario) y el resultado se empuja vía POST /api/billboard/snapshot.
// El servidor sirve, en orden: mem cache → live scrape → snapshot en disco → seed del repo.
const snapshotPath = process.env.RAILWAY_VOLUME_PATH
  ? path.join(process.env.RAILWAY_VOLUME_PATH, 'billboard-snapshot.json')
  : path.join(__dirname, 'cache', 'billboard-snapshot.json');
const seedPath = path.join(__dirname, 'data', 'billboard-seed.json');

// Lee un archivo de películas ({ movies: [...] } o [...] directo). null si falta o es inválido.
function readMoviesFile(filePath) {
  try {
    const parsed = JSON.parse(fs.readFileSync(filePath, 'utf-8'));
    const movies = Array.isArray(parsed) ? parsed : parsed.movies;
    return Array.isArray(movies) && movies.length ? movies : null;
  } catch {
    return null;
  }
}

function writeSnapshot(movies) {
  try {
    fs.mkdirSync(path.dirname(snapshotPath), { recursive: true });
    fs.writeFileSync(snapshotPath, JSON.stringify({ movies, savedAt: new Date().toISOString() }));
  } catch (err) {
    console.error('[Cartelera] No se pudo escribir el snapshot:', err.message);
  }
}

// Cachea en memoria un set ya listo (live o snapshot) durante el TTL vigente.
...
// Cartelera consolidada con la cadena de fallbacks. Deduplica requests concurrentes.
function getBillboardMovies() {
  const now = Date.now();
  if (cinepolisCache.movies && (now - cinepolisCache.fetchedAt) < CINEPOLIS_CACHE_TTL_MS) {
    return Promise.resolve(cinepolisCache.movies);
  }
  if (cinepolisCache.pending) return cinepolisCache.pending;

  cinepolisCache.pending = (async () => {
    try {
      const movies = await scrapeCinepolisMovies();
      await enrichWithTmdb(movies, process.env.TMDB_API_KEY, tmdbMetaCache);
      writeSnapshot(movies); // mantener el snapshot fresco (útil en local / al empujar)
      return cacheMovies(movies);
    } catch (err) {
      // El live scrape falló (típico en prod: WAF 403). Servir el mejor snapshot disponible.
      if (cinepolisCache.movies) {
        console.error('[Cartelera] Scrape falló, sirviendo caché en memoria:', err.message);
        return cinepolisCache.movies;
      }
      const snapshot = readMoviesFile(snapshotPath) || readMoviesFile(seedPath);
      if (snapshot) {
        console.error('[Cartelera] Scrape falló, sirviendo snapshot en disco:', err.message);
        return cacheMovies(snapshot);
      }
      throw err;
    } finally {
...
// Reemplaza el snapshot vigente con datos empujados desde una IP no bloqueada
// (usado por POST /api/billboard/snapshot). Persiste a disco y refresca el caché.
function applyPushedSnapshot(movies) {
  writeSnapshot(movies);
  cacheMovies(movies);
}

// Endpoint para obtener la cartelera consolidada y calificada de Sector Oriente.
// Las películas vienen ya enriquecidas (poster/director/elenco) desde getBillboardMovies;
// aquí solo se adjunta el resumen de puntuaciones locales, siempre fresco.
app.get('/api/billboard', async (req, res) => {
  try {
    const movies = await getBillboardMovies();
...
// Recibe un snapshot fresco de la cartelera empujado desde una IP no bloqueada por
// el WAF de Cinépolis (script scripts/push-billboard.js). Protegido por un secreto
// compartido; ya está exento del bearer-auth por el prefijo /api/billboard.
app.post('/api/billboard/snapshot', (req, res) => {
  const secret = process.env.BILLBOARD_SNAPSHOT_SECRET;
  if (!secret) {
    return res.status(503).json({ error: 'Snapshot no configurado (falta BILLBOARD_SNAPSHOT_SECRET).' });
  }
  if ((req.headers['x-snapshot-secret'] || '') !== secret) {
    return res.status(401).json({ error: 'Secreto de snapshot inválido.' });
  }
  const movies = req.body && req.body.movies;
  if (!Array.isArray(movies) || movies.length === 0) {
    return res.status(400).json({ error: 'Se esperaba { movies: [...] } no vacío.' });
  }
  applyPushedSnapshot(movies);
  console.log(`[Cartelera] Snapshot empujado: ${movies.length} películas.`);
